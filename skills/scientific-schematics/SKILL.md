...
description: >-
  Create publication-quality scientific diagrams using Nano Banana 2 AI with smart iterative refinement. Uses
  Gemini 3.1 Pro Preview for quality review. Only regenerates if quality is below threshold for your document
  type. Specialized in neural network architectures, system diagrams, flowcharts, biological pathways, and complex
  scientific visualizations. 中文触发词：科研示意图、系统架构图、神经网络结构图、流程图、生物通路图、论文插图、publication schematic、模型图、实验流程图。
