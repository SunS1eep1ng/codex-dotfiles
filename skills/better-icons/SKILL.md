...
name: better-icons
description: >-
  Search and extract SVG icons from Iconify collections through the better-icons CLI or MCP. Use when the user
  needs production-ready icons for UI work, not custom illustration design.
  中文触发词：找图标、SVG图标、Iconify、UI图标、按钮图标、图标库、导出SVG、前端图标、替换图标、生产可用icon。
metadata:
