...
name: seaborn
description: >-
  Statistical visualization with pandas integration. Use for quick exploration of distributions, relationships,
  and categorical comparisons with attractive defaults. Best for box plots, violin plots, pair plots, heatmaps.
  Built on matplotlib. For interactive plots use plotly; for publication styling use scientific-visualization.
  中文触发词：seaborn、统计图、箱线图、小提琴图、热力图、pairplot、分布图、pandas可视化、探索性数据分析、快速画图。
