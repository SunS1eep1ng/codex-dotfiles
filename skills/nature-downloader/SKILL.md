...
description: >-
  Use this skill whenever the user wants to configure school/library access, reuse a logged-in Chrome
  institutional session, search library databases, download legitimate open-access or institution-authorized
  academic full text/PDFs, handle missing library permission, organize papers, or read PDFs and supporting
  information. 中文触发词：下载论文、下载PDF、图书馆文献下载、学校登录、机构登录、CARSI、Web of Science、ScienceDirect、合法获取全文、Chrome登录态。
metadata:
