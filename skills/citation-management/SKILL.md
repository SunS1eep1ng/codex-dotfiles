...
description: >-
  Comprehensive citation management for academic research. Search Google Scholar and PubMed for papers, extract
  accurate metadata, validate citations, and generate properly formatted BibTeX entries. This skill should be used
  when you need to find papers, verify citation information, convert DOIs to BibTeX, or ensure reference accuracy
  in scientific writing. 中文触发词：查引用、补引用、生成BibTeX、参考文献管理、DOI转BibTeX、PubMed引用、Google Scholar引用、文献元数据、引用格式、参考文献条目。
