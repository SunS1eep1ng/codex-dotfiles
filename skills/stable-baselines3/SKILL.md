...
description: >-
  ...
  Use for standard RL experiments, quick prototyping, and well-documented algorithm implementations. Best for
  single-agent RL with Gymnasium environments. For high-performance parallel training, multi-agent systems, or
  custom vectorized environments, use pufferlib instead.
  中文触发词：强化学习、stable-baselines3、PPO、SAC、DQN、TD3、A2C、Gymnasium环境、RL实验、智能体训练、策略学习。
license: MIT license
metadata:
